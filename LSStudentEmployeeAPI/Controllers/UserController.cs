...
namespace LSStudentEmployeeAPI.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<ActionResult<User>> GetUser()
        {
            ...
            //List all Users
            var response = client.GetStringAsync("users/41673453").Result;
            if (response !=null)
            {
                var joke = JsonConvert.DeserializeObject<User>(response);
                return joke;
            }
            else
            {
        ...
        {
            var dbEmployee = _mapper.Map<User>(user);

            await _context.Users.AddAsync(dbEmployee);
            await _context.SaveChangesAsync();

            return dbEmployee.account_id;
        }
