using LSStudentEmployeeAPI.Models;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using Newtonsoft.Json;
using System;
using System.Collections.Generic;
using System.Net.Http;
using System.Threading.Tasks;
using System.Net.Http.Headers;

namespace LSStudentEmployeeAPI.Controllers
{
   
    [ApiController]
    public class TimeController : Controller
    {
        private readonly ApplicationDbContext _context;
        //private readonly IMapper _mapper;
        public TimeController(ApplicationDbContext context)
        {
            _context=context;
            //_mapper=mapper;
        }

        [Route("api/times")]
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Time>>> GetTimes()
        {
            var client = new HttpClient();
            client.BaseAddress = new Uri("https://api.wheniwork.com/2/");
            //Add an accept header for JSON format
            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            client.DefaultRequestHeaders.Add("W-Token", "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJhcHAiOjEsImlhdCI6MTUwOTQ5Mjk5OSwibG9naW4iOiI5OTMyNzU3IiwicGlkIjoiOTkzMjc1NyJ9.Zuw42ZLnHKN45QgJWB6O--4WxGlpfKhbRm-Wur9vl2Y5dyw8nAL5okfzj9N9OniwUVBszyU-QJW79TNl1PC_fbRc72His62wGo4rsLkspy2OqiRUrgMqH7G-axs2NkYhE9MtMeC18u-kIvNX7GZTDk1gb6fXhRP64GGiM21Zt-e2rkx6MY39_Q3hFRZwh7m3-ot0jAVMOmHAF12UlHd4OBkUtSuCigAkVdLsVyK_AD2i1qJ_gKwLJNMtZ0x8OXT_ZeNDtJe8tUPOI4nTNWEVaKFaivS_sn3G0_md_bp10LLjXtAfm8-K7sr5NIDvthOeSvpiYCqPqP7nIerX9ycXGQ");
            //List all Users
            var response = client.GetStringAsync("times").Result;

            if (response !=null)
            {

                
                var joke = JsonConvert.DeserializeObject<TimesResponse>(response);

                foreach (var item in joke.times)
                {
                    if (_context.Times.Find(item.id)==null)
                    {
                        _context.Times.Add(item);
                        _context.SaveChanges();
                    }
                    else
                    {
                        var update = _context.Times.Find(item.id);
                       
                        if (update.end_time==null)
                        {
                            _context.Times.Remove(update);
                            _context.Times.Add(item);
                            _context.SaveChanges();
                        }
                        
                    }
                }
                var a = await _context.Times.ToListAsync();
                return a;
            }
            else
            {
                Console.WriteLine("Error");
                return NoContent();
            }
        }



        [HttpGet]
        [Route("api/unadjustedpoints")]
        public async Task<ActionResult<IEnumerable<UnadjustedPoints>>> GetUnPoints()
        {
            foreach (var shift in _context.Shifts)
            {
                foreach (var time in _context.Times)
                {
                    if (shift.id == time.shift_id)
                    {
                        if (DateTime.Parse(shift.start_time).AddMinutes(5) < DateTime.Parse(time.start_time))
                        {
                            
                            var points = new UnadjustedPoints();
                            points.ShiftAssigned=shift;
                            points.TimeInAndOut = time;
                            points.Point=1;
                            _context.UnadjustedPoints.Add(points);
                            _context.SaveChanges();
                        }
                    }
                }
            }

            var a = await _context.UnadjustedPoints.ToListAsync();
            return a;
        }
    }
}
